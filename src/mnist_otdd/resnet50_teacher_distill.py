...
class ResNetDatasetWrapper(Dataset):
...
def train_teacher(dataset_name, train_dataset, test_dataset, output_dir, epochs, batch_size, learning_rate, val_split, seed, device):
    wrapped_train = ResNetDatasetWrapper(train_dataset)
    wrapped_test = ResNetDatasetWrapper(test_dataset)
    ...
    model = ResNet50Teacher().to(device)
    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=0.01)
    loss_fn = nn.CrossEntropyLoss()
    rows = []
    best_test = -1.0
    best_checkpoint = output_dir / f"{dataset_name}_resnet50_best_test.pt"
    for epoch in range(1, epochs + 1):
        ...
        print(f"[resnet50:{dataset_name}] epoch={epoch}/{epochs} val={val_accuracy:.4f} test={test_accuracy:.4f}")
    pd.DataFrame(rows).to_csv(output_dir / f"{dataset_name}_resnet50_metrics.csv", index=False)
    return best_checkpoint


@torch.no_grad()
def collect_teacher_statistics(train_datasets, teacher_paths, device, sample_size_per_dataset):
    teachers = {}
    for name, path in teacher_paths.items():
        teacher = ResNet50Teacher().to(device)
    ...
    for name, dataset in train_datasets.items():
        ...
        loader = DataLoader(ResNetDatasetWrapper(wrapped), batch_size=64, shuffle=False)
...
def synthesize_dataset(teachers, prototypes, spreads, teacher_weights, ipc, steps, lr, feature_weight, entropy_weight, seed, device):
    set_seed(seed)
    synth_images = []
    synth_labels = []
    for label in range(10):
    ...
    return TensorDigitDataset(torch.cat(synth_images, dim=0), torch.cat(synth_labels, dim=0), "ResNet50TeacherSynth")


@torch.no_grad()
def relabel_dataset(dataset, teachers, teacher_weights, temperature, batch_size, device):
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
    images_out, hard_out, soft_out = [], [], []
    for images, hard_labels in loader:
        x = ((images.to(device) * 0.5) + 0.5).repeat(1, 3, 1, 1)
...
def train_student(dataset, test_datasets, epochs, batch_size, learning_rate, alpha, temperature, seed, device, output_dir):
    ...
    cross_eval = pd.DataFrame(evaluate_model_on_all(student, "ResNet50TeacherSimpleCNNStudent", test_datasets, batch_size=batch_size, device=device))
...
def run(args):
    ...
    teacher_paths = {}
    for name in args.datasets:
        teacher_paths[name] = train_teacher(name, train_datasets[name], test_datasets[name], teacher_dir, args.teacher_epochs, args.teacher_batch_size, args.teacher_learning_rate, args.val_split, args.seed, device)
    ...
    cross_eval = train_student(relabeled, test_datasets, args.student_epochs, args.student_batch_size, args.student_learning_rate, args.alpha, args.temperature, args.seed, device, output_dir)
    gap_to_real = compute_distilled_gaps(TensorDigitDataset(relabeled.images, relabeled.hard_labels, "ResNet50TeacherSimpleCNNStudent"), train_datasets, args.gap_sample_size, args.num_projections, args.distance_batch_size, device, output_dir)
...
def parse_args():
    parser = argparse.ArgumentParser(description="ResNet50 teacher distillation with SimpleCNN student.")
